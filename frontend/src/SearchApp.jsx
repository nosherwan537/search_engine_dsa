        ...
        const checkMaintenance = async () => {
            try {
                const response = await axios.get(
                    "http://127.0.0.1:5000/maintenance_status"
                );
    ...
    useEffect(() => {
        const fetchSuggestions = async () => {
            if (query.trim()) {
                try {
                    const response = await axios.get(
                        "http://127.0.0.1:5000/search_suggestions",
                        {
                            params: { query: query },
                        }
                    );
    ...
    return (
        <div
            className={`relative min-h-screen ${
                theme === "bright"
                    ? "bg-gradient-to-br from-purple-500 via-pink-300 to-purple-700 text-black"
            ...
            <div className="absolute inset-0 -z-10 overflow-hidden">
                <div
                    className={`absolute top-0 left-0 w-full h-full ${
                        theme === "bright"
                            ? "bg-gradient-to-b from-purple-300/30 via-transparent to-black/70"
                            : "bg-gradient-to-b from-black/30 via-transparent to-purple-500"
                    }`}
                />
                <motion.div
                ...
                <div
                    className={`absolute inset-0 ${
                        theme === "bright"
                            ? "bg-[radial-gradient(circle,_rgba(255,255,255,0.1)_0%,_rgba(0,0,0,0)_80%)]"
                            : "bg-[radial-gradient(circle,_rgba(0,0,0,0.1)_0%,_rgba(255,255,255,0)_80%)]"
                    }`}
                />
            </div>
                ...
                <form onSubmit={handleSearch} className="mb-10">
                    <div className="relative">
                        <input
                            ...
                            className={`w-full p-5 pr-16 rounded-full ${
                                theme === "bright"
                                    ? "bg-white/30 backdrop-blur-md border border-white/30 text-purple-500 focus:ring-2 focus:ring-purple-400"
                                    : "bg-black/80 backdrop-blur-md border border-black/30 text-purple-500 focus:ring-2 focus:ring-purple-900"
                            } text-lg placeholder-white/50 focus:outline-none transition-all duration-300 shadow-lg hover:shadow-purple-400/30`}
                        />
                        <button
                            type="submit"
                    ...
                    {query.trim() && suggestions.length > 0 && (
                        <div
                            className={`absolute w-full mt-2 z-20 rounded-md backdrop-blur-md border border-white/30 ${
                                theme === "dark"
                                ...
                                {suggestions.map((suggestion, index) => (
                                    <li
                                        ...
                                        className={`px-4 py-2 text-lg cursor-pointer ${
                                            theme === "dark"
                                                ? "text-purple-500 hover:bg-purple-500/50 hover:text-white"
                                                : "text-purple-500 hover:bg-purple-500/50 hover:text-white"
                                        }`}
                                    >
                                        {suggestion}
                ...
                {error && (
                    <motion.p
                        ...
                        className={`text-center font-semibold ${
                            theme === "bright" ? "text-red-400" : "text-red-600"
                        }`}
                    >
                        {error}
                    </motion.p>
                ...
                <AnimatePresence>
                    {results.length > 0 && (
                        <motion.div
                            ...
                            exit={{ opacity: 0, y: 20 }}
                            transition={{ duration: 0.3 }}
                            className={`space-y-4 w-900 backdrop-blur-md ${
                                theme === "bright"
                        ...
                        >
                            {results.map((result, index) => (
                                <motion.div
                                    ...
                                    className={`p-6 rounded-lg shadow-lg hover:scale-105 transition-all ${
                                        theme === "bright"
                                            ? "bg-white/30 hover:bg-pink-200/50 hover:shadow-md hover:shadow-pink-400  backdrop-blur-lg border border-white/30"
                                            : "bg-black/20 hover:bg-purple-600/30 hover:shadow-md hover:shadow-purple-500 backdrop-blur-lg border border-black/30"
                                    }`}
                                >
                                    <h3
                                        ...
                                        <a
                                            ...
                                            className={`flex items-center ${
                                                theme === "bright"
                                                    ...
                                                    : "text-purple-600 hover:text-black/80"
                                            }`}
                                        >
                                            Visit{" "}
